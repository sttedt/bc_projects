...
3. javascript 
 ...
 클레스 : {}, 5가지 형태, let obj = {} <= 객체리터럴,1회성, 인자가 많을 때 사용
...
4. nodejs
 ...
 - 배포 및 운영
  ...
  ===============================================================
   ...
   => 정보보기
    $ pm2 show notJapan
   => 정보보기를 통해서 에러로그, 엑세스(out) 로그의 위치를 볼수 있다.
   => 로그 모니터링
...
==================================================================
  ...
  => 요청이 들어오면 서버에 처리를 분담시킨다. (라운드로빈 : 1->2->3->4->5->.....)
  => 로드벨런스 작업
...
7. 리눅스
 ...
 - 기본 os 업데이트
  ...
  => shell(쉘) -> 리눅스상에서 명령어 처리기, 텍스트, 한줄씩 순차적으로 수행 vs bat파일(윈도우)
    -> bash(배쉬), sh, zbash(좀더이쁜)
    -> 쉘코드의 첫줄 -> #!/bin/bash : bash로 쉘스크립트를 구동하라는 의미 
  $ 노드 설치
  ...
  8. 실전 프로젝트 
    - 목적 
      ...
      > 데이터 구축을 위한 웹스크래핑 진행
        ...
        => 1단계 : 공공데이터, 데이터센터에서 제공, 통계제공, 국가제공 => 정제된 데이터
        => 2단계 : openApi를 이용한 수집(포털, 특수목적,...)         =>정제된 데이터
        => 3단계 : 웹스크래핑(webscraping) : 웹을 요청해서 사이트를 긁어서 html에서 데이터를 추출
                  => css selector가 필수 시굴, DOM(Document Object Model) 조작
        => 4단계 : 매크로, 드루킹(예를들면 악하게 사용되면), 티켓자용예매(악하게 사용되면) 
                    => 크롤링(crawling), 브라우저에서 사람이 하는 행위처럼 구성하여 
                      자동화 시켜서 데이터를 수집하는 행위(로그인, 글쓰기, 데이터 추출 다됨)
                      => 유튜브, 로그인 후 처리해야하느 사이트 => ajax를 사용하는 사이트가 주대상
...
$ node>cd notJapan
$ npm install
  -> package.json을 읽어서 dependencies에 존재하는모듈을 설치
